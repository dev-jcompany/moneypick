...
const titleMap: Record<string, string> = {
  ...
  '/admin/content/new': '새 콘텐츠 등록',
  '/admin/categories': '카테고리 설정',
  '/admin/subscribers': '뉴스레터 신청자 관리',
  '/admin/notices': '공지사항 목록',
  '/admin/notices/new': '공지사항 등록',
  '/admin/calculators': '계산기 변수 설정',
  '/admin/users': '운영자 계정 관리',
};

export default function AdminHeader() {
  const pathname = usePathname();
  const title = pathname.startsWith('/admin/content/') && pathname !== '/admin/content/new' ? '콘텐츠 수정' : pathname.startsWith('/admin/notices/') && pathname !== '/admin/notices/new' ? '공지사항 수정' : titleMap[pathname] ?? '머니픽 관리자';
  return (
