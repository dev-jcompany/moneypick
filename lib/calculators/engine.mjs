// 머니픽 계산기 공식 — 브라우저(CalculatorClient.tsx)와 서버 스크립트(mcp/scheduled-generator.mjs)에서
// 동일한 로직을 재사용하기 위해 순수 함수로 분리했다. React/Next.js 의존성 없음.
//
// @typedef {Record<string, number | string | boolean>} Values
// @typedef {{ label: string; value: number; suffix?: string; emphasize?: boolean }} Result
...
function annuityAnnual(principal, annualRate, years) {
...
function brokerageCap(type, amount) {
  if (type === 'sale') {
 ...
 * @returns {Result[]}
 */
export function calculate(slug, v) {
  ...
  if (slug === 'gift-tax') {
    ...
    const brackets = [[10000000, 0.1, 0], [50000000, 0.2, 1000000], [100000000, 0.3, 6000000], [300000000, 0.4, 16000000], [Infinity, 0.5, 56000000]];
    let tax = 0;
...
// ── 자동 사례 생성용 프리셋 (Phase 2 ①) ──────────────────────────────
// 콘텐츠에 많이 쓰이는 계산기부터 우선 지원. 새 계산기를 추가하려면 이 객체에
// { slug: [{ name, values }, ...] } 형태로 프리셋만 추가하면 된다.
const SCENARIO_PRESETS = {
  dsr: [
    { name: '30대 · 연봉 5,200만원 · 신규대출 2억원', values: { income: 52000000, existing: 0, newLoan: 200000000, rate: 4.5, years: 30, limit: 40 } },
    { name: '40대 · 연봉 7,000만원 · 기존상환 800만원 · 신규대출 3억원', values: { income: 70000000, existing: 8000000, newLoan: 300000000, rate: 4.5, years: 30, limit: 40 } },
    { name: '사회초년생 · 연봉 3,600만원 · 신규대출 1억원', values: { income: 36000000, existing: 0, newLoan: 100000000, rate: 4.8, years: 30, limit: 40 } },
  ],
  'acquisition-tax': [
    { name: '4억 아파트 · 1주택', values: { price: 400000000, houseCount: 'one', area: 'adj' } },
    { name: '6억 아파트 · 1주택', values: { price: 600000000, houseCount: 'one', area: 'adj' } },
    { name: '10억 아파트 · 1주택', values: { price: 1000000000, houseCount: 'one', area: 'adj' } },
  ],
  mortgage: [
    { name: '3억원 · 금리 4.0% · 30년 원리금균등', values: { principal: 300000000, rate: 4.0, years: 30, repayType: 'annuity' } },
    { name: '5억원 · 금리 4.5% · 20년 원리금균등', values: { principal: 500000000, rate: 4.5, years: 20, repayType: 'annuity' } },
  ],
  'jeonse-loan': [
    { name: '전세보증금 3억 · 자기자금 1억', values: { deposit: 300000000, cash: 100000000, rate: 4.0, ltv: 80 } },
    { name: '전세보증금 5억 · 자기자금 1.5억', values: { deposit: 500000000, cash: 150000000, rate: 4.2, ltv: 80 } },
  ],
};

/**
 * 실제 계산기 공식으로 산출한 "사례" 목록을 반환한다. AI가 숫자를 새로 지어내지 않고
 * 이 결과를 그대로 서술에 활용하도록 하기 위한 것 — 프리셋이 없는 slug는 빈 배열.
 * @param {string} slug
 * @returns {{ slug: string, name: string, inputs: Values, results: Result[] }[]}
 */
export function generateSampleScenarios(slug) {
  const presets = SCENARIO_PRESETS[slug];
  if (!presets) return [];
  return presets.map((preset) => ({
    slug,
    name: preset.name,
    inputs: preset.values,
    results: calculate(slug, preset.values),
  }));
}

export const SCENARIO_SUPPORTED_CALCULATORS = Object.keys(SCENARIO_PRESETS);

/**
 * 결과값을 사람이 읽는 형식으로("6,000,000원", "23.4%") 포맷한다.
 * CalculatorClient.tsx의 표시 규칙과 동일하게 유지한다.
 * @param {Result} result
 */
export function formatResultValue(result) {
  const suffix = result.suffix ?? (/요율|DSR|경감률/.test(result.label) ? '' : '원');
  return `${new Intl.NumberFormat('ko-KR').format(Math.round(result.value))}${suffix}`;
}
