...
import { CANONICAL_CONTENT_TYPES } from '../article-system/content-types.mjs';

export const ARTICLE_TYPES = CANONICAL_CONTENT_TYPES;
...
function classBlock(html, className) {
  ...
  const match = String(html ?? '').match(new RegExp(`<([a-z][a-z0-9]*)[^>]*class=["'][^"']*\\b${escaped}\\b[^"']*["'][^>]*>([\\s\\S]*?)<\\/\\1>`, 'i'));
  return match?.[2] ?? '';
}
...
function normalizeCalculators(value) {
  ...
  for (const item of value) {
    ...
    if (known && !result.some((entry) => entry.href === known.href)) {
      result.push({ label: label || known.label, href: known.href });
    }
