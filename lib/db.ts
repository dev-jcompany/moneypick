...
function serverClient() {
  ...
  if (!serviceRoleKey) {
    throw new Error('[security] SUPABASE_SERVICE_ROLE_KEY is required for server database operations.');
  }
  return createClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    serviceRoleKey,
...
export async function createContactInquiry(
  ...
  try {
    ...
    if (error) {
      console.error('[db] createContactInquiry:', error.code ?? 'DATABASE_ERROR');
      return { id: null, error: 'Database insert failed', code: error.code };
    }
    return { id: data?.id ?? null };
  } catch (e) {
    console.error('[db] createContactInquiry exception:', e instanceof Error ? e.name : 'UnknownError');
    return { id: null, error: 'Database insert failed', code: 'DATABASE_EXCEPTION' };
  }
}
...
export async function getContactInquiries(limit = 100): Promise<{ inquiries: ContactInquiry[]; error?: string; code?: string }> {
  try {
    ...
    if (error) {
      console.error('[db] getContactInquiries:', error.code ?? 'DATABASE_ERROR');
      return { inquiries: [], error: 'Database query failed', code: error.code };
    }

    return { inquiries: (data ?? []) as ContactInquiry[] };
  } catch (e) {
    console.error('[db] getContactInquiries exception:', e instanceof Error ? e.name : 'UnknownError');
    return { inquiries: [], error: 'Database query failed', code: 'DATABASE_EXCEPTION' };
  }
}
